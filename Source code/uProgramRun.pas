...
begin
  ...
  Begin
    ...
    If (MazeStart.X = 0) and (MazeStart.Y = 0) then
      ShowMessage('Òðåáóåòñÿ ñèìâîë "S" - start (íà÷àëî) íà êàðòå');

    If (MazeEnd.X = MapHeight-1) and (MazeEnd.Y = MapWidth-1) then
      ShowMessage('Òðåáóåòñÿ ñèìâîë "E" - end (êîíåö) íà êàðòå');

    If (LowerCase(Map[MazeStart.X][MazeStart.Y]) = 'x')
      or (LowerCase(Map[MazeEnd.X][MazeEnd.Y]) = 'x') then
      ShowMessage('Óáåäèòåñü, ÷òî òàêîé ëàáèðèíò ìîæíî ðåøèòü');
...
// Sender - activated object
begin
  ENameEnter(EMazeHeight, 'îò 3 äî 100');
end;
...
// Sender - activated object
Begin
  ENameExit(EMazeHeight, 'îò 3 äî 100');
End;
...
// Sender - activated object
Begin
  ENameEnter(EMazeWidth, 'îò 3 äî 100');
End;
...
// Sender - activated object
Begin
  ENameExit(EMazeWidth, 'îò 3 äî 100');
End;
